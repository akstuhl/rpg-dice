...
module.exports = {
    ...
    rollDice (dice, sides, usingFate) {
        let total = 0;
        for (let i = 0; i < dice; i++) {
            if (usingFate) total += Math.floor(Math.random() * 3) - 1;
            else total += Math.floor(Math.random() * sides) + 1;
        }
        return total;
    },
    ...
    roll () {
        ...
        if (editor) {
            ...
            if (matches = selection.match(regex[0])) {                          // attack rolls, ability checks, damage rolls, etc.
                ...
                mod = (sign === '-' ? mod * -1 : mod);
                roll = this.rollDice(dice, sides, fateDice);
                res = roll + mod;
