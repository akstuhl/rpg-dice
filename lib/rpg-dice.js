...
module.exports = {
    ...
    roll() {
        ...
        const regex = [
            /*/(?:[^\d]*\d* +\(?)? (?:(?<dice>\d+)?d(?<sides>\d+))? *(?:(?<sign>[+-]) *(?<modifier>\d+)?)?/,*/
            /^(?:(?<dice>\d+)?d(?<sides>\d+))? *(?:(?<sign>[\+]?|-)* *(?<modifier>\d+)?)?$/i,
            /^STATS?(?::(?<method>\d)(?:-(?<count>\d+))?)?$/i
        ];
        let dice = atom.config.get('rpg-dice.dND');
        let sides = atom.config.get('rpg-dice.dDS');
        let sign = '+';
        let mod = 0;
        let method = atom.config.get('rpg-dice.dSGM');
        let count = atom.config.get('rpg-dice.dRC');
        let type, roll, res, output;

        var roller = function(dice, sides) {
            let total = 0;
        ...
        var stats = function(method, count=defaultCount) {
        ...
        if (editor) {
            ...
            if (matches = selection.match(regex[0])) {                          // attack rolls, ability checks, damage rolls, etc.
                ...
                output = sprintf(output).padStart(5);
